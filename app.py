...
def _run_single_correction(addr1, addr2, addr3, city, state, country, postal, use_ai=False):
    ...
    # Step 3b — US state code with blank country → assume US
    ...
    # Step 4b — authoritative US state from ZIP prefix
    # Fires when: country is US, OR country is blank, OR state is a US code.
    # This is the final word — overrides any wrong state the user entered.
    zip_state = infer_us_state_from_zip(c_postal)
    _is_us = c_country == "US" or c_country == "" or c_state.upper() in _US_STATE_CODES
